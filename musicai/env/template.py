...
class Template_Gym(gym.Env):
    ...
    def __init__(self, env=Template()):
        ...
        if self.discrete:
            ...
            self.action_space = space.MultiBinary(128)

            # observation is the x, y coordinate of the grid
            #low = np.zeros(0, dtype=int)
            #high =  np.array(1, dtype=int) - np.ones(len(self.maze_size), dtype=int)
            self.observation_space = spaces.Box(low=0, high=100, shape=(128,))
        else:
            ...
            self.observation_space = spaces.Box(low=0, high=100, shape=(128,), dtype=np.float16)
